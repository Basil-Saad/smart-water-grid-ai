...
void setup() {
  ...
  Serial.println("Node 3 starting...");
...
void loop() {
  ...
  if (millis() - lastTime >= 1000) {
    ...
    // ----- SEND TO FIREBASE -----
    Database.set<float>(asyncClient, "/nodes/node3/sensors/consumption", consumption);

    // ----- READ COMMANDS FROM FIREBASE -----
    bool valveState = Database.get<bool>(asyncClient, "/nodes/node3/actuators/valve");
    ...
    // ----- CHECK RESET COMMAND -----
    bool resetCmd = Database.get<bool>(asyncClient, "/nodes/node3/commands/resetConsumption");
    if (resetCmd) {
      consumption = 0;
      hourStartTime = millis();
      Serial.println("Consumption reset by dashboard!");
      Database.set<bool>(asyncClient, "/nodes/node3/commands/resetConsumption", false);
    }
  }
}